...
const ElixrModel: React.FC<ElixrModelProps> = ({ pair, streamData }) => {
	...
	function calculateElixrIntersection(
		elixrMax: { intercept: number; slope: number },
		elixrMin: { intercept: number; slope: number },
	) {
		if (
			elixrMax.slope === elixrMin.slope ||
			(elixrMax.slope < elixrMin.slope &&
				elixrMax.intercept < elixrMin.intercept) ||
			(elixrMax.slope > elixrMin.slope &&
				elixrMax.intercept > elixrMin.intercept)
		) {
			return null;
		...
		const xIntersection =
			(elixrMin.intercept - elixrMax.intercept) /
			(elixrMax.slope - elixrMin.slope);
		const intersectingPrice =
			elixrMax.slope * xIntersection + elixrMax.intercept;

		return intersectingPrice;
	}

	const calculateElixrs = (
		oandaData: CandleData[],
	): { elixrMax: Elixr[]; elixrMin: Elixr[] } => {
		...
		const touchedElixrMax = countTouches(highs, elixrMax).filter(
			(elixr) => elixr.touches >= 10,
		);
		const touchedElixrMin = countTouches(lows, elixrMin).filter(
			(elixr) => elixr.touches >= 10,
		);
	...
	const generateMasterElixrsAndUpdatePrice = (oandaData: CandleData[]) => {
		...
		const intersectPrice = calculateElixrIntersection(
			averageElixrMax,
			averageElixrMin,
		);
	...
	const updatePriceToElixrRatio = () => {
		if (
			trendlines &&
			trendlines.elixrMax.length > 0 &&
			trendlines.elixrMin.length > 0
		) {
			const maxElixrLastIndex = trendlines.elixrMax.length - 1;
			const minElixrLastIndex = trendlines.elixrMin.length - 1;
			const maxElixrPrice =
				trendlines.elixrMax[0].slope * maxElixrLastIndex +
				trendlines.elixrMax[0].intercept;
			const minElixrPrice =
				trendlines.elixrMin[0].slope * minElixrLastIndex +
				trendlines.elixrMin[0].intercept;
				...
				<>
					...
					<div className="w-full flex justify-center items-center gap-2">
						<Button onClick={toggleBot}>
							{botActive ? "Turn Off Elixr" : "Turn On Elixr"}
						</Button>
					</div>
				</>
			) : (
