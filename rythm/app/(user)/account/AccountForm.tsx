...
export default function AccountForm() {
  ...
  const [existingCredentials, setExistingCredentials] = useState({ apiKey: '', accountId: '' });
  ...
  return (
    <div className="flex justify-center items-center w-full">
      <div className="w-full max-w-xl bg-black rounded-lg shadow-md p-6 mt-6">
        <div className="mb-4">
          <h2 className="font-mono text-lg text-white mb-2">Oanda Credentials</h2>
          {existingCredentials.apiKey && <p className="text-gray-400">Current API Key: ****{existingCredentials.apiKey}</p>}
          {existingCredentials.accountId && <p className="text-gray-400 mb-4">Account ID: {existingCredentials.accountId}</p>}
        </div>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-6">
            ...
            <input id="apiKey" name="apiKey" type="text" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Enter new API Key" className="font-mono shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
          </div>
